...
function renderOrganizationActions(organization) {
  return `<div class="table-actions"><button class="btn btn-small" data-org-interaction="${esc(organization.id)}" data-crm-permission="crm.contact_followup.create">新增互动</button><button class="btn btn-small" data-org-lead="${esc(organization.id)}" data-crm-permission="crm.lead.create">创建线索</button><button class="btn btn-small" data-org-task="${esc(organization.id)}" data-crm-permission="crm.task.create">创建任务</button><button class="btn btn-small" data-org-nurture="${esc(organization.id)}" data-crm-permission="crm.organization.nurture.manage">开始孵化</button><button class="btn btn-small" data-view-org="${esc(organization.id)}">查看</button></div>`;
}
...
function renderOrganizationTable(rows, vendor = false) {
  ...
  if (vendor) {
    return `<div class="crm-table-scroll"><table class="crm-data-table organization-table vendor-table"><thead><tr><th>Logo</th><th>公司</th><th>联系人</th><th>Website</th><th>地区</th><th>负责人</th><th>最近互动</th><th>备注</th><th>操作</th></tr></thead><tbody>${rows.map((organization) => `<tr data-view-org="${esc(organization.id)}"><td>${logoMarkup(organization, "small")}</td><td><strong>${esc(organization.name)}</strong><small>${esc(valueOrDash(organization.shortName))}</small></td><td>${esc(organization.contactCount)}</td><td>${esc(valueOrDash(organization.website))}</td><td>${esc([organization.country, organization.region, organization.city].filter(Boolean).join(" · ") || "-")}</td><td>${esc(valueOrDash(organization.owner?.name))}</td><td>${esc(formatLocalDateTime(organization.lastInteractionAt))}</td><td>${esc(valueOrDash(organization.note))}</td><td>${renderOrganizationActions(organization)}</td></tr>`).join("")}</tbody></table></div>`;
  }
  return `<div class="crm-table-scroll"><table class="crm-data-table organization-table"><thead><tr><th>Logo</th><th>公司</th><th>角色</th><th>生命周期</th><th>Fit</th><th>Engagement</th><th>负责人</th><th>联系人</th><th>活跃线索</th><th>最近互动</th><th>下一动作</th><th>更新时间</th><th>操作</th></tr></thead><tbody>${rows.map((organization) => `<tr data-view-org="${esc(organization.id)}"><td>${logoMarkup(organization, "small")}</td><td><strong>${esc(organization.name)}</strong><small>${esc(valueOrDash(organization.shortName))}</small></td><td>${organization.roleKeys.map((role) => badge(role, roleLabels[role])).join(" ")}</td><td>${badge(organization.lifecycleStage, lifecycleLabels[organization.lifecycleStage])}</td><td><strong>${esc(organization.fitScore)}</strong> ${badge(organization.fitLevel, levelLabels[organization.fitLevel])}</td><td><strong>${esc(organization.engagementScore)}</strong> ${badge(organization.engagementState, engagementStateLabels[organization.engagementState])}</td><td>${esc(valueOrDash(organization.owner?.name))}</td><td>${esc(organization.contactCount)}</td><td>${esc(organization.activeLeadCount)}</td><td>${esc(formatLocalDateTime(organization.lastInteractionAt))}</td><td>${esc(formatLocalDateTime(organization.nextActionAt))}</td><td>${esc(formatLocalDateTime(organization.updatedAt))}</td><td>${renderOrganizationActions(organization)}</td></tr>`).join("")}</tbody></table></div>`;
}

async function organizationPrimaryContact(organizationId) {
  const result = await crmApi(`/api/v1/crm/organizations/${organizationId}`);
  const contacts = result.data.contacts || [];
  if (contacts.length === 1) return contacts[0];
  if (!contacts.length) context.notify("该公司暂无联系人，请先新增或关联联系人");
  else { context.notify("该公司有多个联系人，请在公司详情中选择联系人"); context.navigate(`organizations/${organizationId}`); }
  return null;
}
...
function bindOrganizationRows(container, rows) {
  ...
  container.querySelectorAll("[data-org-interaction]").forEach((button) => button.addEventListener("click", async (event) => { event.stopPropagation(); const contact = await organizationPrimaryContact(button.dataset.orgInteraction); if (contact) openFollowup({ kind: "contact", id: contact.id, title: `${contact.contactName} · 互动`, onSaved: loadCustomerOperations }); }));
  container.querySelectorAll("[data-org-lead]").forEach((button) => button.addEventListener("click", async (event) => { event.stopPropagation(); const contact = await organizationPrimaryContact(button.dataset.orgLead); if (contact) context.openLeadForm(contact); }));
...
export async function loadOrganizations() {
  ...
  $("orgSearch").addEventListener("click", refresh); ["orgKeyword", "orgIndustry"].forEach((id) => $(id).addEventListener("keydown", (event) => { if (event.key === "Enter") refresh(); }));
...
function renderTaskCards(tasks) {
  return `<div class="task-queue">${tasks.map((task) => `<article class="task-card ${task.status === "OPEN" && new Date(task.dueAt) < new Date() ? "is-overdue" : ""}"><header><span>${badge(task.status, taskStatusLabels[task.status])}${task.priority === "HIGH" ? badge("HIGH","高优先") : ""}</span><time>${esc(formatLocalDateTime(task.dueAt))}</time></header><h3>${esc(task.title)}</h3><p>${esc(task.description || task.organization?.name || task.contact?.contactName || "-")}</p><footer><span>${esc(task.owner?.name || "-")}</span>${task.status === "OPEN" ? `${task.leadId ? `<button class="btn btn-small" data-task-followup="${task.id}" data-crm-permission="crm.lead_followup.create">新增跟进</button>` : task.contactId ? `<button class="btn btn-small" data-task-followup="${task.id}" data-crm-permission="crm.contact_followup.create">新增跟进</button>` : ""}<button class="btn btn-small" data-task-postpone="${task.id}" data-crm-permission="crm.task.edit">顺延 1 天</button><button class="btn btn-primary btn-small" data-task-complete="${task.id}" data-crm-permission="crm.task.complete">完成</button>` : ""}</footer></article>`).join("") || `<div class="empty-state crm-compact-empty"><div><h3>暂无任务</h3></div></div>`}</div>`;
}
...
export async function loadWorkbench() {
  const view = $("workbenchView"); view.innerHTML = `<div class="page ops-page"><header class="page-heading"><div><span class="eyebrow">Daily Execution</span><h1>我的工作台</h1><p>今天要联系谁、为什么联系、下一步做什么。</p></div><button class="btn btn-primary" id="newStandaloneTask" data-crm-permission="crm.task.create"><svg><use href="#i-plus"/></svg>创建任务</button></header><section class="content-card"><div class="crm-detail-loading"><span class="crm-skeleton crm-skeleton-block"></span></div></section></div>`;
  ...
  bindTaskActions(view, tasks, loadWorkbench); $("newStandaloneTask")?.addEventListener("click", () => context.notify("请从公司、联系人或线索创建任务，以保证任务有关联对象")); context.applyCrmPermissions();
}
...
export async function loadCustomerOperations() {
  ...
  if (operationsTab === "nurture") {
    $("operationsContent").innerHTML = `<div class="list-card-header"><div><h2>孵化池</h2><p>每个计划都有原因、目标、节奏、负责人、下一触达与主题</p></div></div><div class="nurture-grid">${nurtures.map((item) => `<article class="nurture-card"><header>${badge(item.status,item.status)}<time>${esc(formatLocalDateTime(item.nextTouchAt))}</time></header><h3>${esc(item.organization.name)}</h3><dl><div><dt>为什么</dt><dd>${esc(item.reason)}</dd></div><div><dt>目标</dt><dd>${esc(item.objective)}</dd></div><div><dt>下次主题</dt><dd>${esc(item.touchTopic)}</dd></div><div><dt>负责人 / 节奏</dt><dd>${esc(item.owner.name)} · ${item.cadenceDays} 天</dd></div></dl><footer>${item.status === "ACTIVE" ? `<button class="btn btn-small" data-nurture-pause="${item.id}" data-crm-permission="crm.organization.nurture.manage">暂停</button><button class="btn btn-primary btn-small" data-nurture-complete="${item.id}" data-crm-permission="crm.organization.nurture.manage">完成</button>` : `<button class="btn btn-small" data-nurture-resume="${item.id}" data-crm-permission="crm.organization.nurture.manage">恢复</button>`}<button class="btn btn-small" data-view-org="${item.organizationId}">查看公司</button></footer></article>`).join("") || "暂无孵化计划"}</div>`;
...
function dashboardAnalyticsQuery() {
  if ($("dashboardPeriod").value !== "custom") return analyticsQuery(Number($("dashboardPeriod").value));
  const fromValue = $("dashboardFrom").value;
  const toValue = $("dashboardTo").value;
  if (!fromValue || !toValue) return analyticsQuery(30);
...
export async function loadDashboard() {
  ...
  const today = new Date(); const defaultFrom = new Date(today.getTime() - 30 * 86_400_000); const dateValue = (date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2,"0")}-${String(date.getDate()).padStart(2,"0")}`;
  view.innerHTML = `<div class="page ops-page dashboard-page"><header class="page-heading"><div><span class="eyebrow">${management ? "Management Overview" : "Personal Overview"}</span><h1>Dashboard</h1><p>用非金额指标判断客户资产、机会推进与团队执行是否健康。</p></div><div class="dashboard-filters"><select class="control" id="dashboardPeriod"><option value="7">近 7 天</option><option value="30" selected>近 30 天</option><option value="90">近 90 天</option><option value="custom">自定义</option></select><div class="dashboard-custom-range" id="dashboardCustomRange" hidden><label>开始日期<input class="control" id="dashboardFrom" type="date" value="${dateValue(defaultFrom)}"></label><label>结束日期<input class="control" id="dashboardTo" type="date" value="${dateValue(today)}"></label></div>${management ? `<select class="control" id="dashboardOwner"><option value="">全部负责人</option>${context.getUsers().map((user) => `<option value="${user.id}">${esc(user.name)}</option>`).join("")}</select>` : ""}<select class="control" id="dashboardRole"><option value="">潜客 + 客户</option><option value="PROSPECT">潜在客户</option><option value="CUSTOMER">客户</option></select></div></header><div id="dashboardContent"><div class="crm-detail-loading"><span class="crm-skeleton crm-skeleton-block"></span></div></div></div>`;
  const refresh = async () => { const params = dashboardAnalyticsQuery(); if (management && $("dashboardOwner").value) params.set("ownerUserId", $("dashboardOwner").value); if ($("dashboardRole").value) params.set("organizationRole", $("dashboardRole").value); const [analyticsResult,matrixResult,teamResult] = await Promise.all([crmApi(`/api/v1/crm/analytics/${management ? "management" : "self"}?${params}`), crmApi(`/api/v1/crm/analytics/fit-engagement-matrix?${params}`), management ? crmApi(`/api/v1/crm/analytics/team?${params}`) : Promise.resolve({ data: { rows: [] } })]); renderDashboard(analyticsResult.data,matrixResult.data,teamResult.data); };
  $("dashboardPeriod").addEventListener("change", () => { $("dashboardCustomRange").hidden = $("dashboardPeriod").value !== "custom"; refresh(); }); $("dashboardFrom").addEventListener("change", refresh); $("dashboardTo").addEventListener("change", refresh); $("dashboardOwner")?.addEventListener("change", refresh); $("dashboardRole").addEventListener("change", refresh); await refresh();
}
