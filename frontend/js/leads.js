...
export function readonlyContactMarkup(contact, compact = false) {
  if (!contact) return "";
  return `<div class="crm-readonly-contact${compact ? " is-compact" : ""}"><header><div><span>所属 Contact · 只读</span><strong>${esc(contact.contactName)}</strong></div><span class="crm-lock-label"><svg><use href="#i-lock"/></svg>Automatic Field</span></header><div class="crm-readonly-grid"><div><span>公司</span><strong>${esc(contact.companyShortName || contact.companyName || "-")}</strong></div><div><span>职位</span><strong>${esc(contact.title || "-")}</strong></div><div><span>Email</span><strong>${esc(contact.email || "-")}</strong></div><div><span>Phone</span><strong>${esc(contact.phone || "-")}</strong></div><div><span>行业</span><strong>${esc(contact.industry || "-")}</strong></div><div><span>国家 / 城市</span><strong>${esc([contact.country, contact.city].filter(Boolean).join(" / ") || "-")}</strong></div></div></div>`;
}
...
export function initializeLeads(options) {
  ...
  $("crmLeadsView").innerHTML = `<div class="crm-page">
    ...
    <section class="crm-filter-bar crm-lead-filters" aria-label="Lead 筛选">
      ...
      <button class="crm-icon-button" id="crmSearchLeads" type="button" title="查询" aria-label="查询 Leads"><svg><use href="#i-search"/></svg></button>
    ...
    <section class="crm-table-section" aria-labelledby="crmLeadTableTitle">
      <header><div><h2 id="crmLeadTableTitle">Lead 目录</h2><span id="crmLeadResultCount">0 条</span></div></header>
      <div id="crmLeadListState" class="crm-list-state"></div>
      <div class="crm-table-scroll" id="crmLeadTableWrap"><table class="crm-data-table crm-lead-table"><thead><tr><th>项目需求简述</th><th>Contact / 公司</th><th>状态</th><th>优先级</th><th>销售负责人</th><th>跟进负责人</th><th>下次跟进</th><th>最近跟进</th><th>预计报价</th><th>更新时间</th></tr></thead><tbody id="crmLeadRows"></tbody></table></div>
      <div class="crm-empty" id="crmLeadEmpty" hidden><svg><use href="#i-lead"/></svg><h3>暂无 Leads</h3><p>创建 Lead 后，可集中维护需求、负责人和跟进记录。</p><button class="btn btn-primary" id="crmEmptyNewLead" type="button" data-crm-permission="crm.lead.create"><svg><use href="#i-plus"/></svg>New Lead</button></div>
  ...
  document.body.insertAdjacentHTML("beforeend", `<aside class="crm-drawer" id="crmLeadDrawer" aria-hidden="true">
    <button class="crm-drawer-backdrop" type="button" data-close-lead-form aria-label="关闭 Lead 表单"></button>
    <form class="crm-drawer-panel" id="crmLeadForm" novalidate>
      <header class="crm-drawer-header"><div><span class="crm-eyebrow">LEAD</span><h2 id="crmLeadFormTitle">New Lead</h2></div><button class="crm-icon-button" type="button" data-close-lead-form aria-label="关闭"><svg><use href="#i-x"/></svg></button></header>
      <div class="crm-drawer-body">
        <section class="crm-contact-picker" id="crmLeadContactPicker"><header><h3>选择 Contact</h3><span>必选</span></header><label class="crm-search"><svg><use href="#i-search"/></svg><input id="crmLeadContactKeyword" type="search" placeholder="搜索联系人、公司、Email 或手机号"></label><div class="crm-contact-options" id="crmLeadContactOptions"></div></section>
      ...
      <footer class="crm-drawer-footer"><button class="btn" type="button" data-close-lead-form>取消</button><button class="btn btn-primary" id="crmSaveLead" type="submit">保存 Lead</button></footer>
    </form>
  </aside>`);
...
export async function openLead(id) {
  ...
  try {
    ...
    container.innerHTML = `<div class="crm-record">
      <header class="crm-record-header"><button class="crm-icon-button" id="crmBackToLeads" type="button" title="返回 Leads" aria-label="返回 Leads"><svg><use href="#i-arrow"/></svg></button><div class="crm-record-title"><span class="crm-eyebrow">LEAD</span><h1>${esc(lead.requirementSummary)}</h1><p>${esc(lead.contact.contactName)} · ${esc(lead.contact.companyShortName || lead.contact.companyName || "-")}</p></div><span class="crm-badge crm-status-${esc(lead.status.toLowerCase())}">${esc(leadStatusLabel(lead.status))}</span><span class="crm-badge crm-priority-${esc(lead.priority.toLowerCase())}">${esc(leadPriorityLabel(lead.priority))}</span><div class="crm-record-actions"><button class="btn" id="crmEditLead" type="button" data-crm-permission="crm.lead.edit"><svg><use href="#i-edit"/></svg>编辑 Lead</button></div></header>
      <section class="crm-record-band"><header><div><span class="crm-section-index">R</span><h2>Requirement</h2></div></header><div class="crm-description-grid">${requirementMarkup(lead)}</div></section>
      <section class="crm-record-band"><header><div><span class="crm-section-index">F</span><h2>Followup</h2><small>${followupResult.meta.total} 条记录</small></div><button class="btn btn-small" id="crmAddLeadFollowup" type="button" data-crm-permission="crm.lead_followup.create"><svg><use href="#i-plus"/></svg>Add Followup</button></header><div class="crm-description-grid crm-followup-settings">${followupSettingsMarkup(lead)}</div><div class="crm-timeline">${renderTimelineMarkup(followupResult.data, "lead")}</div></section>
      <section class="crm-record-band"><header><div><span class="crm-section-index">A</span><h2>Contact</h2><small>Automatic Field · Read only</small></div><button class="btn btn-small" id="crmViewLeadContact" type="button">View Contact<svg class="crm-forward-icon"><use href="#i-chevron"/></svg></button></header>${readonlyContactMarkup(lead.contact, true)}</section>
    </div>`;
    ...
    $("crmAddLeadFollowup").addEventListener("click", () => openFollowup({ kind: "lead", id: lead.id, title: "新增 Lead 跟进", onSaved: () => openLead(lead.id) }));
    $("crmViewLeadContact").addEventListener("click", () => context.navigate(`contacts/${lead.contact.id}`));
    context.applyCrmPermissions();
    return lead;
  } catch (error) {
    container.innerHTML = `<div class="crm-detail-error">${renderErrorMarkup(error, "crmRetryLeadDetail")}<button class="btn" id="crmErrorBackLeads" type="button"><svg><use href="#i-arrow"/></svg>返回 Leads</button></div>`;
...
export async function openLeadForm(contact = null, lead = null) {
  ...
  $("crmLeadFormTitle").textContent = lead ? "编辑 Lead" : "New Lead";
...
async function searchContacts(keyword) {
  ...
  try {
    ...
    $("crmLeadContactOptions").innerHTML = result.data.length ? result.data.map((contact) => `<button class="crm-contact-option" type="button" data-picker-contact="${esc(contact.id)}"><span><strong>${esc(contact.contactName)}</strong><small>${esc(contact.companyShortName || contact.companyName || "-")}</small></span><span>${esc(contact.email || contact.phone || "-")}</span><svg><use href="#i-chevron"/></svg></button>`).join("") : '<div class="crm-inline-empty">没有匹配的 Contact</div>';
...
function selectContact(contact) {
  ...
  $("crmSelectedContact").innerHTML = `${readonlyContactMarkup(contact)}<button class="btn btn-small crm-change-contact" id="crmChangeLeadContact" type="button">更换 Contact</button>`;
...
async function saveLead(event) {
  ...
  if (!applyFieldErrors(form, errors)) {
    errorNode.textContent = errors.contactId || "请检查标记的 Lead 字段。";
  ...
  try {
    ...
    closeLeadForm();
    context.notify(wasEditing ? "Lead 已更新" : "Lead 已创建");
