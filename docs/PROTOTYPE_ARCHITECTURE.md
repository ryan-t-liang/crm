...
## Marketing activities — UI Rebuild V4
...
Activity V4 follows the binding kivisense-admin-design skill and KIVISENSE_DESIGN_SYSTEM_V2.md. The same centered 760px Semi Modal creates/edits core activity information; no creation SideSheet, wizard, publication checklist, slot, draw-rule or prize configuration is included. Detail uses compact identity/time/location metadata, four line tabs (概览 / 活动设置 / 参与管理 / 中奖与核销) and lighter text subnav. Overview groups source-identical drilldown records into participation, draw and claim summaries rather than independent metric cards. Settings have a left-aligned 1040px maximum content width; reservation/draw rules retain focused editors. Activity reservation data uses ACTIVITY only; prize reservations have their own prize-bookings leaf and PRIZE-only data view. Prior booking links remain readable without moving stored records. Scoped SAVE_ACTIVITY merge, operational validation, immutable award/code allocation and locking remain unchanged.
