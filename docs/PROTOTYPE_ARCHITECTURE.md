...
Lottery scope is selected once at activity creation: ACTIVITY uses one activity prize pool; SESSION uses only the participant's booked session pool. Missing, omitted or disabled session entries never fall back to activity probabilities. Scope cannot be switched in the same activity. Pre-scope local records with explicit session rows use SESSION uniformly; other records use ACTIVITY. This is a read adapter, not a rewrite of historical draws, awards or snapshots. A previously mixed legacy activity may therefore have unconfigured sessions which must be configured before further draws. Session mode currently requires reservation participation to identify a unique session. Activity information, booking rules and activity slots are edited together in one validated save; lottery timing/limits can be edited separately.
