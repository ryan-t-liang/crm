...
## Marketing activities — UI Refinement V3
...
Activity detail uses four primary areas (overview, configuration, participation records, prize fulfillment) with scoped secondary tabs. One 720px Semi SideSheet creates/edits only the activity's core information; no wizard, publication checklist, slot, draw-rule or prize configuration is part of creation. Created activities expose separate reservation, draw-rule and prize configuration, dynamically shown for their capabilities; old leaf routes remain readable. Scoped SAVE_ACTIVITY calls merge only their basic/reservation/draw fields into the current activity so unrelated configuration is not overwritten by a form snapshot. Existing operational validation, immutable award/code allocation and locking still apply.
