...
需预约权益在查看旁增加预约记录，按当前activity + participation + award过滤PRIZE历史，保留取消 / 改约记录和未预约空态；不混入ACTIVITY。活动预约行若已有需预约奖项，同样提供该参与者领奖预约历史入口。参与用户与实际Redemption分别保留只读查看入口，核销执行仍仅在独立Staff Surface。旧leaf route只读适配到新三个页签或对应历史抽屉，不移动存储记录。
