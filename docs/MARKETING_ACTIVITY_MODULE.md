...
CRM 唯一营销主入口 `#marketing` 是活动列表，不再有顶层奖品库、预约记录、核销工作台。列表含名称、原品牌、线上 / 线下、时间、管理状态与独立阶段、预约 / 抽奖开关、预约 / 参与 / 中奖人数，支持查看、编辑、复制、发布、暂停 / 恢复、取消及“更多”进入同一活动数据 Tab。举行日期筛选是与活动时间区间相交，不是新增统计。
...
| 抽奖 & 奖品 | 抽奖规则、活动内奖品、库存、概率、履约承诺、兑换码导入 |
...
`#redemption/<credential>` 是独立核销端，没有 CRM AppShell / 侧栏 / 后台导航，不管理活动配置。兼容原 `#marketing/redemption/<credential>`，同样渲染独立页面。ACT / WIN 随机标识使用实际二维码编码，不包含姓名、手机号或后台权限。CRM 核销数据页只读。
...
没有全局奖品主数据集合。同名 A / B 奖品仍是不同 ID、库存、代码及 slots。复制仅配置，产生新活动 / 奖品 / 场次 ID，不复制参与、预约、机会、抽奖、中奖、核销、审计或库存占用。兑换码库及分配不复制，复制后重新导入唯一代码才能发布。
...
代码只在本地读取，CSV 可带 code / 兑换码表头；拒绝空码、批次重复、多列歧义及跨活动 / 奖品重复，拒绝时整批不导入。导入量低于配置库存不可发布；已发布可先追加代码，再追加配额。一次状态提交同步保存 draw、award、扣次、库存占用、assignedAwardId / assignedAt 和内容快照。重复 operationId、点击、刷新返回原内容，不重分配代码。
...
发布检查各奖品概率 + NONE = 100%，不归一化。固定概率下，库存 / 个人上限 / 履约容量耗尽的区间变成 NONE，不重新分配给其他奖品。无任何有效可中奖库存、代码、容量或概率时停抽，不扣次。
...
| 已领取 / 发放 | 有 fulfilledAt 或 issuedAt 的 award |
| 剩余配额 | max(0, quota − 所有 award 数)，中奖即占用，领取不二扣；过期不返池 |
| 兑换码可继续中奖 | min(剩余配额, 未分配代码数) |
| 预约型新可中奖 | min(剩余配额, max(0, 有效场次剩余容量 − 未预约中奖承诺)) |

有效履约 slots 必须尚可预约且未结束；剩余容量已扣现有预约 / 履约占用。未履约、未过期、没有有效预约的旧赢家仍占履约承诺，避免后来的赢家抢走保障。取消 PRIZE 预约释放座位，不取消 award、库存占用或未预约承诺。库存可大于时段容量，但不能超额中奖。新增配额 / 时段单独留 Audit；已发布规则锁定，不覆盖历史中奖快照。
...
八指标：当前有效活动预约人数、到场人数、完成人数、抽奖人数、抽奖次数、中奖人数、中奖份数、已领取 / 已发放份数。人数按本活动 participation 去重，次数按 draw（含 NONE），份数按 award；取消不计有效预约，虚拟发放口径明确。点击消费相同活动 / 品牌 / 授权与固定查看时间的原始 ID 数组；明细冻结打开时间，数据 / 角色 / 活动 / Tab 变化关闭。营销参与不加进集团总人数，不改 Dashboard 口径。

现有 HQ_ADMIN 具有既有 GP / UN 范围，允许管理 / 核销 / 预览；分销商无会员 / 营销品牌权限。动作层分开 brands / manage / redeem / preview；列表、详情、预览及新旧核销 URL 均先授权。未新增品牌管理员、核销员账号或真实后台权限。
...
本地同步提交与幂等不是生产事务或服务器安全。真实接入需后端权威身份 / 品牌权限、事务库存 / 预约 / 代码、可信随机、不可变历史与恢复策略；本轮不搭建。Safari / Firefox / 真机 / 扫描硬件及多用户压力未验；单品牌 / 只读授权以规则夹具验证，不假称已有页面账号。31 项场景的实际层级见 `MARKETING_ACTIVITY_ACCEPTANCE.md`。
