...
Activity Detail 固定使用以下一级和二级入口：

| 一级入口 | 二级入口 | 职责 |
| --- | --- | --- |
| 概览 | — | 活动表现、抽奖情况、领奖情况三组摘要及必要业务上下文 |
| 活动设置 | 基本信息 / 预约设置 / 抽奖设置 / 奖品设置 | 活动配置、活动场次和场次级奖品控制；不适用项可隐藏 |
| 参与管理 | 参与用户 / 预约记录 / 抽奖记录 | Flexible Participation、ACTIVITY预约与每次Draw事实 |
| 中奖与核销 | 中奖记录 / 领奖预约 / 核销记录 | Award、PRIZE预约与真实Redemption事实 |

详情复用成熟 Lead / Deal 的 `DetailWorkspace`、`record-tabs` 主工作表面、Semi Table 和可选 `SideSection` / `DataList` 信息栏。右侧只放活动编号、品牌、类型、参与方式、状态及活动 / 预约 / 抽奖时间等真实业务上下文。布局、字号、间距、Tag、Card、Tabs 和编辑入口全部遵循 `DESIGN_SYSTEM.md`，本模块不另设视觉规则。

概览不建立 KPI Card 墙。活动表现显示参与人数、到场人数、完成人数、中奖人数；抽奖情况显示抽奖人数、抽奖次数、中奖份数；领奖情况显示待领取、已预约、已领取。直接参与、未启用预约或未启用抽奖时，保留适用入口并展示明确空态，不制造业务记录。

新记录由集中创建 / 复制动作记录可选 `createdBy`，编辑不能覆盖；旧创建人显示未记录。可选 `identity.gender` 是活动表单快照，不是 Sowind 字段，不从会员称谓推断，旧缺失值不回填。

抽奖读侧阶段继续保留：1未抽奖、2已抽奖、3已核销（无需预约）、4已预约、5已核销（需要预约）。3 / 5 中文相同但值 / 流程独立；未中奖仍为2，直接虚拟发放不冒充外部核销。参与用户、预约、Draw、Award、PRIZE预约与Redemption分别进入上述业务入口，不合并或改写底层事实。详见 `MARKETING_RECORD_FIELDS.md`。
...
```text
奖品名称 / 奖品类型 / 领取方式 / 数量模式 / 可发放数量 / 默认中奖概率
```
...
| --- | --- |
| 可发放数量 | `quantityLimit` |
| 已分配数量 | 所有该奖品 `SessionPrize.allocatedQuantity` 之和 |
| 未分配数量 | `quantityLimit - 已分配数量` |
| 已中奖数量 | 当前 Activity 下该 Prize 的 Award 数量 |
| 剩余可发放数量 | `quantityLimit - 已中奖数量` |

已中奖数量已经包含在场次分配内，不能从“未分配”再次扣减。活动设置 → 奖品设置同时展示以上口径；“未分配”和“剩余可发放”不得混用。中奖即占用可发放数量，领取、过期或取消领奖不二次扣减，也不自动返还。
...
```text
sum(all SessionPrize.allocatedQuantity) <= ActivityPrize.quantityLimit
```

本场分配不能低于本场已中奖，增加不能超过活动未分配数量。低于已中奖时提示“当前场次已有15份中奖记录，本场可发放数量不能低于15份。”；超过当前可用数量时提示“当前奖品仅剩8份未分配数量。”。减少只释放尚未中奖的数量回活动未分配，不删除 Award。关闭尚未中奖的奖品时将概率和本场分配归零；已有中奖时将概率归零、分配收敛到 `sessionWonCount`、本场剩余归零，历史 Award 保持不变。

新场次初始分配为0，不隐式占用活动全部数量。可显式“复制其他场次配置”，只复制启用状态、概率和分配数量，不复制 Draw、Award、已中奖或剩余结果；复制后仍校验活动未分配数量，不能自动超配，数量不足时提示“当前可分配数量不足，请调整本场数量。”。

活动设置 → 预约设置 → 活动场次的主表只展示日期、时间、场地、预约人数 / 容量、奖品配置摘要、状态和操作。摘要使用“3个奖品 · 剩余62份”或“未配置”，不永久铺开概率矩阵。管理奖品进入中等 Contextual Drawer，逐行显示参与本场、中奖概率、本场分配、已中奖和剩余，并提供取消 / 保存。
...
ActivityPrize 的 `defaultProbability` 只用于直接参与活动的实际抽奖概率，以及创建新 SessionPrize 时的初始模板。已存在的 SessionPrize 是显式配置；以后修改默认概率不得静默覆盖场次配置。
...
场次尚未开始且没有历史 Draw 时，可以直接编辑 enabled、probability和allocatedQuantity。场次已开始或已有 Draw 后，数量通过“调整数量”表达增加 / 减少的 delta；使用小 Modal / Popover，同时展示当前分配、已中奖、当前剩余、活动未分配和调整后结果。

- 增加数量占用活动未分配数量，不能超过当前可分配上限；
- 减少数量释放未中奖部分，调整后分配不能低于 `sessionWonCount`；
...
实体直接领取：待领取 → 已领取。预约型权益：待预约 → 已预约 / 待领取 → 已领取；体验完成标“已履约（体验完成）”，过期权益保留占用与历史。直接虚拟码标“兑换码已分配”、凭证标“权益已生成”、链接标“领取链接已生成”，均指平台本地发放，不伪造外部兑换 / 已使用 / 已查看。概览、中奖表、Staff识别和会员只读记录复用同一履约判定。
