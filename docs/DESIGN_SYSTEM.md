...
- 已经由产品确认的详情页信息架构不能在纯视觉精修中被重新分组、增加一级页签或改造成新的导航层级。

## 创建与编辑

- 简单业务对象优先使用 Quick Create / Edit Modal，在不离开当前工作区的前提下完成核心记录创建或轻量编辑；
- 当前页面上下文内较复杂的查看或编辑使用 Drawer / SideSheet；完整 Record Detail 使用独立详情页；
- 创建与编辑尽量使用相同字段顺序和表单结构；
- 普通表单使用现有 `form-stack` 或 `form-grid`，保持一到两列；
- Header、关闭、滚动区和 Footer 不做模块级重新设计；
- 从 Dropdown 打开 Modal / SideSheet 时先关闭菜单；子面板打开时隐藏父面板并保留上下文，避免多个可操作抽屉叠在一起；
- 一个表单通常只有取消和一个主要提交动作；
- Contextual SideSheet 的核心字段应通过复合信息单元、合理列宽或分组在抽屉内完整阅读，不能依赖横向滚动完成主要任务；
- Modal 同样可用于确认或短暂聚焦的只读内容；它不是所有创建流程的强制容器；
- 只有流程存在强顺序、前一步决定后一步且无法合理先创建对象时，才使用 Wizard / Stepper；不能默认引入全屏向导或嵌套页面。
...
- 表格行操作默认保留一个常用动作，其余动作进入 More；不要把同权重操作全部平铺；
- 已经产生历史事实的运行中数量不能用普通编辑直接覆盖，应提供明确的“调整”动作并展示调整前后结果；
- 重复配置的批量生成应先预览数量与结果，再确认提交；表格用复合单元格组合日期 / 时间、已用 / 剩余等同一含义的信息，技术标识放详情而不是铺满主表；
