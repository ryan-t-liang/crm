...
> 文档角色：字段映射与只读业务口径。它不定义通用页面结构、视觉 token 或组件样式。
...
按最新要求，活动详情Header统一提供“活动管理”下拉菜单：创建奖品 / 编辑活动信息 / 编辑预约设置 / 管理场次 / 抽奖设置；卡片编辑图标、管理场次按钮与表格创建按钮不再重复出现。各入口复用原右侧SideSheet，创建奖品从任何页签打开，保存后进入奖品设置。管理能力只受角色与品牌授权限制，不因发布、结束、取消或暂未开启对应能力而置灰；关闭的预约 / 抽奖能力只保存配置，不自动启用。
...
SAVE_ACTIVITY_PRIZE允许追加独立新奖品；既有已发布 / 有业务记录奖品的实质规则和已分配兑换码仍保护，中奖快照不重建。SAVE_ACTIVITY_SLOT不再因活动结束 / 取消或能力关闭阻止配置，保留场次窗口、容量及历史保护；结束活动的新场次初始时间建议落在原活动窗口，而非自动挪到今天。新的奖品默认概率0；若调整后概率合计不是100%，实际抽奖会阻止且不扣次数。
