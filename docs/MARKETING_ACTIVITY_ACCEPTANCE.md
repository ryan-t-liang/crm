...
> 文档角色：按时间累积的历史验收证据，不是当前 Design System，也不能覆盖成熟 CRM 页面或 `DESIGN_SYSTEM.md`。

## 统一活动管理菜单与配置可操作性 — 2026-09-18

基线82e85ab。原创建 / 编辑入口变灰来自发布历史锁定与CANCELED条件，不是缺组件。现改为Header一个“活动管理”菜单，含创建奖品 / 编辑活动信息 / 编辑预约设置 / 管理场次 / 抽奖设置；右侧摘要卡片和奖品表不重复放入口。五个配置操作按管理与品牌权限开放，结束 / 取消后仍可配置，不自动重开活动。所有数据录入仍从右侧滑出。

定向编辑使用既有SAVE_ACTIVITY.section字段白名单，状态、发布 / 创建元数据和历史参与 / 预约 / 次数 / 抽奖 / 中奖 / 核销记录不变；既有品牌、使用过的场次时间 / 地点、安全容量、已分配兑换码与旧奖品实质规则受保护。原整对象锁定断言继续保留，不借菜单绕过批量写入保护。允许新奖品 / 场次追加，结束活动建议时间保留原活动窗口。新增奖品默认概率0，概率未配平时阻止实际抽奖、不扣次。

按用户免回归要求，不运行本地test或浏览器QA，只运行类型/lint、构建和发布资产验证；已有组件断言适配新菜单，两项同文件规则断言覆盖取消活动可配置及历史 / 品牌 / 容量保护，交由已有推送CI自动执行，不新增QA套件。实际完成结果见最终交付报告；历史下方灰色 / 禁止创建的结论已被本次明确授权替代。

## 标题14px、创建奖品入口与预约表展示 — 2026-09-18

基线0aa8089。页签 / 右侧卡片标题统一14px，正文和既有间距不变。奖品设置增加“创建奖品”按钮，复用右侧FormSideSheet与现有字段和保存校验；已发布或有业务历史的活动保留规则锁定，创建面板提示限制并禁用提交。活动预约列表与查看详情的参与时段只显示时间；状态所在网格使用内容对齐，标签不拉伸整列。销售 / 会员Store、业务模型、示意数量与LocalStorage均未改。

遵守用户免回归要求，不执行本地test或浏览器QA；本轮只执行类型/lint、构建和静态发布资产验证。原组件断言仅更新按钮文案，不修改权限断言。GitHub已有CI自动运行；实际完成结果以最终交付报告为准，下面历史视觉与测试结论不代表本次调整。

追加卡片编辑入口修正：共享SideSection增加可选onEdit / editDisabled / editLabel，使用小号无边框Semi编辑图标与Tooltip，保留旧actions调用兼容。现有三个有顶部编辑入口的活动信息 / 预约设置 / 抽奖设置卡片均改用图标，权限与锁定条件、点击后的原侧栏编辑流程不变。组件断言通过aria-label定位原编辑动作，不再依赖已移除的可见按钮文字；不减少编辑和无写入断言。
