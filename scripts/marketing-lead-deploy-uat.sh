...
if [[ "$mode" = prepare ]]; then
  ...
  test "$after" -eq 7
  docker exec sowind-crm-test-mysql-1 sh -c 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot -N kivisense_crm_uat -e "SELECT migration_name FROM _prisma_migrations WHERE migration_name = '\''20260907010000_marketing_lead_opportunity_conversion'\'' AND finished_at IS NOT NULL AND rolled_back_at IS NULL; SELECT '\''marketing_leads'\'', COUNT(*) FROM marketing_leads UNION ALL SELECT '\''lead_activity_events'\'', COUNT(*) FROM lead_activity_events UNION ALL SELECT '\''lead_score_history'\'', COUNT(*) FROM lead_score_history UNION ALL SELECT '\''lead_status_history'\'', COUNT(*) FROM lead_status_history;"' > "$backup/marketing-schema-after.tsv"
