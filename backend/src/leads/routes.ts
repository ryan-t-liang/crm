...
export const leadInputSchema = z.object({
  ...
  phone: z.preprocess((value) => value === "" ? undefined : value, z.string().trim().min(8).max(40).nullable().optional()), language: z.string().trim().max(32).default("zh"),
  ...
  ownsBrandWatch: z.enum(["Yes", "No", "是", "否"]).nullable().optional(),
  birthday: z.preprocess((value) => value === "" ? undefined : value, z.coerce.date().nullable().optional()),
...
export async function leadRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/v1/leads", { preHandler: guard("lead.view") }, async (request) => {
    ...
    const statusGroups = await app.prisma.lead.groupBy({ by: ["syncStatus"], where, _count: { syncStatus: true }, orderBy: { syncStatus: "asc" } });
    const byStatus = Object.fromEntries(statusGroups.map((group) => [group.syncStatus, group._count.syncStatus])) as Record<string, number>;
    const metrics = {
      leadTotal: total,
      statuses: {
        NOT_SYNCED: byStatus.NOT_SYNCED ?? 0,
        SYNC_PENDING: byStatus.SYNC_PENDING ?? 0,
        SYNCING: byStatus.SYNCING ?? 0,
        GATEWAY_ACCEPTED: byStatus.GATEWAY_ACCEPTED ?? 0,
        SYNC_FAILED: byStatus.SYNC_FAILED ?? 0,
        DEAD_LETTER: byStatus.DEAD_LETTER ?? 0,
      },
      pending: (byStatus.NOT_SYNCED ?? 0) + (byStatus.SYNC_PENDING ?? 0) + (byStatus.SYNCING ?? 0),
      gatewayAccepted: byStatus.GATEWAY_ACCEPTED ?? 0,
      syncExceptions: (byStatus.SYNC_FAILED ?? 0) + (byStatus.DEAD_LETTER ?? 0),
    };
    return { data: rows, meta: paginationMeta(query.page, query.pageSize, total), metrics };
  });

  app.post("/api/v1/leads", { preHandler: guard("lead.create") }, async (request, reply) => {
    const parsed = leadInputSchema.parse(request.body);
    const body = { ...parsed, source: "ADMIN_MANUAL" as const, submissionMode: "ADMIN_MANUAL" as const };
    ...
    const result = await app.prisma.$transaction(async (tx) => {
      ...
      if (!created.duplicate) await appendAudit(tx, request, { action: "CREATE_LEAD", module: "lead", targetType: "lead", targetId: created.lead.id, brandId: brand.id, details: { source: body.source, submissionMode: body.submissionMode, outboxCreated: created.outboxCreated } });
      return created;
    });
  ...
  app.post<{ Params: { id: string } }>("/api/v1/leads/:id/sync", { preHandler: guard("lead.sync") }, async (request) => {
    ...
    await app.prisma.$transaction(async (tx) => {
      ...
      await tx.lead.update({ where: { id: lead.id }, data: { syncStatus: "SYNC_PENDING", lastSyncError: null } });
    ...
    return { data: { id: lead.id, syncStatus: "SYNC_PENDING" } };
  });
