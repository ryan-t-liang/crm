...
import type { SowindLeadInput } from "./sowind.types.js";
...
export async function reactivateLeadOutbox(db: DbClient, leadId: string): Promise<void> {
  await db.integrationOutbox.updateMany({
    ...
    data: { status: "PENDING", attempts: 0, nextAttemptAt: new Date(), lockedAt: null, lastError: null, processedAt: null },
  });
}
