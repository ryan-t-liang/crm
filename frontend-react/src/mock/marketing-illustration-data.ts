import type { MarketingActivity, MarketingAward, MarketingParticipation, MarketingParticipationChannel, MarketingState } from "@/types/marketing";
...
export function appendMarketingIllustrations(state: MarketingState, activity: MarketingActivity, now: number) {
  ...
  for (let index = 0; index < 30; index++) {
    ...
    state.bookings.push({ id: `${prefix}:booking`, activityId: activity.id, participationId: participant.id, kind: "ACTIVITY", slotId: slot.id,
