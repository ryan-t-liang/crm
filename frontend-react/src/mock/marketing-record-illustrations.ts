...
export function createMarketingRecordIllustrations(activity: MarketingActivity, now: number): MarketingState {
  ...
  sample.id = namespace; sample.name = activity.name; sample.location = activity.location || "示意活动场地";
  sample.slots = [];
...
export function withMarketingRecordIllustrations(input: MarketingState, sample: MarketingState): MarketingState {
  ...
  return {
    ...input,
    activities: input.activities.map(activity => activity.id === fixture.id ? { ...activity, slots: [...activity.slots, ...fixture.slots], pool: [...activity.pool, ...fixture.pool] } : activity),
