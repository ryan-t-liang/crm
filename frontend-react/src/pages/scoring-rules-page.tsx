...
export function ScoringRulesPage({ me }: { me: SessionUser }) {
  ...
  const columns: ColumnDef<LeadScoringRule>[] = [
    ...
    { accessorKey: "category", header: "规则分类" }, { id: "scoreDimension", header: "评分维度", cell: ({ row }) => row.original.scoreDimension === "FIT" ? "线索匹配度" : "互动活跃度" },
  ...
  return <PageContent><PageHeader title="评分规则" description="配置营销行为对线索匹配度和互动活跃度的影响；历史分值不会被回写。" />{result.error ? <ErrorState error={result.error} retry={result.reload} /> : <DataTable label="评分规则" rows={rows} columns={columns} loading={result.loading} primaryAction={manageable ? <Button onClick={() => setEditing(true)}><Plus />新增规则</Button> : undefined} />}{editing && <RuleForm rule={editing === true ? undefined : editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); result.reload(); }} />}</PageContent>;
}
