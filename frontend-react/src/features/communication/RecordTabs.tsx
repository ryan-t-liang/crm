...
import { FormSideSheet, StatusTag } from "@/components/CrmUi";
...
function Emails({ entityType, entityId }: { entityType: EntityType; entityId: string }) {
  const { state, sendEmail, currentUser, canWrite } = useCrm();
  ...
  const submit = () => {
    ...
    const saved = sendEmail({ entityType, entityId, threadId: rows[0]?.threadId || `thread-${entityId}`, to: form.to.split(",").map((value) => value.trim()), cc: form.cc ? form.cc.split(",").map((value) => value.trim()) : [], bcc: form.bcc ? form.bcc.split(",").map((value) => value.trim()) : [], subject: form.subject, body: form.body, attachmentNames: form.attachmentNames });
    if (saved) { setOpen(false); Toast.success("邮件已发送"); }
  };
  return <div className="tab-panel"><PanelHeader title="Email Thread" description="完整模拟邮件线程，不连接外部邮箱。" action={<Button disabled={!canWrite} theme="solid" icon={<IconMail />} onClick={() => reply()}>New Email</Button>} />
    <div className="email-thread">{rows.length ? rows.map((message) => <article className="email-message" key={message.id}><header><div><strong>{message.from === currentUser.email ? currentUser.name : message.from}</strong><small>to {message.to.join(", ")}</small></div><time>{dateTime(message.sentAt)}</time></header><h3>{message.subject}</h3><p className="email-body">{message.body}</p>{message.attachmentNames.length > 0 && <div className="attachment-chips">{message.attachmentNames.map((name) => <span key={name}>{name}</span>)}</div>}<footer><Button disabled={!canWrite} theme="borderless" size="small" onClick={() => reply(message)}>Reply</Button><Button disabled={!canWrite} theme="borderless" size="small" onClick={() => reply(message, true)}>Reply All</Button></footer></article>) : <Empty title="暂无邮件" description="点击 New Email 创建一条可回复的模拟邮件线程。" />}</div>
    <FormSideSheet visible={open} title="New Email" className="email-composer" width={720} onCancel={() => setOpen(false)} onOk={submit} okText="Send" cancelText="Cancel">
...
function Comments({ entityType, entityId }: { entityType: EntityType; entityId: string }) {
  const { state, addComment, updateComment, deleteComment, canWrite } = useCrm();
  ...
  return <div className="tab-panel"><PanelHeader title="Internal Comments" description="使用 @Name 提及团队成员；评论同时进入 Activity。" />
    <div className="comment-compose"><TextArea value={body} onChange={setBody} placeholder="添加内部评论，例如：@Jason 请确认下周演示时间" autosize={{ minRows: 3, maxRows: 6 }} /><Button theme="solid" disabled={!canWrite || !body.trim()} onClick={() => { if (addComment({ entityType, entityId, body })) setBody(""); }}>发布评论</Button></div>
    <div className="record-list">{rows.length ? rows.map((item) => <article key={item.id}><header><strong>{state.users.find((user) => user.id === item.authorId)?.name}</strong><time>{dateTime(item.createdAt)}</time></header>{editing === item.id ? <Input defaultValue={item.body} onEnterPress={(event) => { updateComment(item.id, event.currentTarget.value); setEditing(""); }} /> : <p>{item.body}</p>}<footer><Button disabled={!canWrite} theme="borderless" size="small" icon={<IconEdit />} onClick={() => setEditing(item.id)}>编辑</Button><Button disabled={!canWrite} theme="borderless" size="small" type="danger" icon={<IconDelete />} onClick={() => Modal.confirm({ title: "删除评论？", content: "此操作只影响当前浏览器中的 Demo 数据。", onOk: () => { deleteComment(item.id); } })}>删除</Button></footer></article>) : <Empty title="暂无评论" />}</div>
  </div>;
}
...
function Calls({ entityType, entityId, contactId, distributorId }: { entityType: EntityType; entityId: string; contactId: string; distributorId: string }) {
  const { state, addCall, canWrite } = useCrm(); const [open, setOpen] = useState(false);
  ...
  const submit = () => { if (!form.summary.trim() || !Number.isFinite(Date.parse(form.occurredAt))) { Toast.warning("请填写通话摘要和有效时间"); return; } const saved = addCall({ entityType, entityId, contactId: form.contactId, occurredAt: new Date(form.occurredAt).toISOString(), direction: form.direction as "INBOUND" | "OUTBOUND", durationMinutes: form.durationMinutes, summary: form.summary, result: form.result, nextAction: form.nextAction }); if (saved) setOpen(false); };
  return <div className="tab-panel"><PanelHeader title="Calls" description="记录来电、去电、联系人、时间、结果与下一步行动。" action={<Button disabled={!canWrite} icon={<IconPlus />} onClick={() => setOpen(true)}>Add Call</Button>} /><div className="record-list">{rows.length ? rows.map((item) => <article key={item.id}><header><strong>{item.direction === "INBOUND" ? "Inbound" : "Outbound"} · {item.durationMinutes} min</strong><time>{dateTime(item.occurredAt)}</time></header><p>{item.summary}</p><small>{state.contacts.find((contact) => contact.id === item.contactId)?.name} · {item.result} · Next: {item.nextAction || "—"}</small></article>) : <Empty title="暂无电话记录" />}</div><FormSideSheet visible={open} title="Add Call" onCancel={() => setOpen(false)} onOk={submit}><div className="form-stack"><label>Direction<Select aria-label="Direction" value={form.direction} onChange={(value) => setForm((current) => ({ ...current, direction: String(value) }))} optionList={[{ value: "INBOUND", label: "Inbound" }, { value: "OUTBOUND", label: "Outbound" }]} /></label><label>Contact<Select aria-label="Contact" filter value={form.contactId} onChange={(value) => setForm((current) => ({ ...current, contactId: String(value) }))} optionList={contacts.map((item) => ({ value: item.id, label: item.name }))} /></label><label>Date / Time<Input aria-label="Date / Time" type="datetime-local" value={form.occurredAt} onChange={(value) => setForm((current) => ({ ...current, occurredAt: value }))} /></label><label>Duration (min)<Input aria-label="Duration (min)" type="number" value={String(form.durationMinutes)} onChange={(value) => setForm((current) => ({ ...current, durationMinutes: Number(value) }))} /></label><label>Summary<TextArea aria-label="Summary" value={form.summary} onChange={(value) => setForm((current) => ({ ...current, summary: value }))} /></label><label>Result<Input aria-label="Result" value={form.result} onChange={(value) => setForm((current) => ({ ...current, result: value }))} /></label><label>Next Action<Input aria-label="Next Action" value={form.nextAction} onChange={(value) => setForm((current) => ({ ...current, nextAction: value }))} /></label></div></FormSideSheet></div>;
}

function Tasks({ entityType, entityId, distributorId }: { entityType: EntityType; entityId: string; distributorId: string }) {
  const { state, addTask, updateTask, canWrite } = useCrm(); const [open, setOpen] = useState(false);
  ...
  const submit = () => { if (!form.title.trim() || !Number.isFinite(Date.parse(form.dueAt))) { Toast.warning("请填写任务名称和有效时间"); return; } const saved = addTask({ title: form.title, ownerId: form.ownerId, distributorId, relationType: entityType, relationId: entityId, dueAt: new Date(form.dueAt).toISOString(), priority: form.priority as CrmTask["priority"], status: "OPEN", description: "Created from record workspace" }); if (saved) setOpen(false); };
  return <div className="tab-panel"><PanelHeader title="Tasks" description="创建、分配、完成或取消与当前记录关联的任务。" action={<Button disabled={!canWrite} icon={<IconPlus />} onClick={() => setOpen(true)}>Create Task</Button>} /><div className="task-list">{rows.length ? rows.map((item) => <article key={item.id}><div><StatusTag value={item.status} label={item.status} /><strong>{item.title}</strong><small>{state.users.find((user) => user.id === item.ownerId)?.name} · {dateTime(item.dueAt)}</small></div><div>{item.status === "OPEN" && <><Button disabled={!canWrite} size="small" onClick={() => updateTask(item.id, { status: "DONE" })}>Complete</Button><Button disabled={!canWrite} size="small" type="tertiary" onClick={() => updateTask(item.id, { status: "CANCELED" })}>Cancel</Button></>}</div></article>) : <Empty title="暂无关联任务" />}</div><FormSideSheet visible={open} title="Create Task" onCancel={() => setOpen(false)} onOk={submit}><div className="form-stack"><label>Title<Input aria-label="Title" value={form.title} onChange={(value) => setForm((current) => ({ ...current, title: value }))} /></label><label>Owner<Select aria-label="Owner" value={form.ownerId} onChange={(value) => setForm((current) => ({ ...current, ownerId: String(value) }))} optionList={owners.map((user) => ({ value: user.id, label: user.name }))} /></label><label>Due Date<Input aria-label="Due Date" type="datetime-local" value={form.dueAt} onChange={(value) => setForm((current) => ({ ...current, dueAt: value }))} /></label><label>Priority<Select aria-label="Priority" value={form.priority} onChange={(value) => setForm((current) => ({ ...current, priority: String(value) }))} optionList={["LOW", "MEDIUM", "HIGH"].map((value) => ({ value, label: value }))} /></label></div></FormSideSheet></div>;
}

export function EntityNotes({ entityType, entityId }: { entityType: RecordEntityType; entityId: string }) {
  const { state, addNote, updateNote, deleteNote, canWrite } = useCrm(); const [title, setTitle] = useState(""); const [body, setBody] = useState(""); const [editing, setEditing] = useState("");
