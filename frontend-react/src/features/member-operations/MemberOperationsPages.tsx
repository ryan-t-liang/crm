...
function dictionary(value: string | null): ReactNode {
...
function MemberCustomerList() {
  ...
  return <div className="page member-workspace"><PageHeader title="集团客户" description="customer 集团主档；SQL 文件缺失，因此本轮不推测 customer 的姓名、等级、积分或会员编码字段。" actions={<BrandScopeSelect />} />
...
function MemberCustomerDetail({ id }: { id: string }) {
  ...
  return <DetailWorkspace eyebrow="会员与品牌运营 / customer" title={customer.id} subtitle="集团客户主档（仅展示已确认字段）" backRoute="member-customers" tags={<>{users.map((user) => <BrandTag key={user.id} brand={user.brand} />)}{new Set(users.map((user) => user.brand)).size > 1 && <Tag color="amber" size="small">跨品牌关联</Tag>}</>} actions={<BrandScopeSelect />} tabs={<Tabs className="record-tabs">
    <TabPane tab="集团主档" itemKey="overview"><section className="data-panel"><PageHeader title="customer" description="未读取到 Sowind SQL，不把原型展示信息冒充为 customer 表字段。" /><DataList rows={[["customer.id", customer.id], ["关联 user 数", users.length], ["关联购买意向数", intents.length]]} /></section></TabPane>
...
function BrandMemberList() {
  ...
  const columns = [
    ...
    { title: "国家码 + 手机号", width: 175, render: (_: unknown, item: SowindBrandUser) => phone(item.country_code, item.phone) },
...
function PurchaseIntentList() {
  ...
  const columns = [
    ...
    { title: "国家码 + 手机号", width: 175, render: (_: unknown, item: SowindPurchaseIntent) => phone(item.country_code, item.phone) },
