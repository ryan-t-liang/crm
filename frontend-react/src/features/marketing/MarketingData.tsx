...
import { awardFulfillmentLabel, bookingLabels, bookingStatus, chances, marketingPermissions, needsReservation, participantChannel, participantDisplayName, participantIdentity, participantIdentityReview, participationIssue, prizeTypeLabels, redemptionLabels, slotFor } from "./marketing-model";
import { DefinitionGrid, displayDate, Panel, SelectField, TextField } from "./MarketingUi";

export const channelLabels = { WECHAT_MINIPROGRAM: "微信小程序", WECHAT_H5: "微信 H5", WEB_H5: "网页 H5", QR_H5: "线下二维码", STAFF: "现场登记", OTHER: "其他" };
...
export function ParticipantTable({ activity }: { activity: MarketingActivity }) {
  ...
  return <><div className="marketing-toolbar"><TextField label="搜索参与用户" value={filters.search} onChange={filters.setSearch} /><SelectField label="参与状态" value={filters.status} onChange={filters.setStatus} list={[{ value: "ALL", label: "全部状态" }, ...Object.entries(participantLabels).map(([value, label]) => ({ value, label }))]} /><TextField label="参与创建日期" type="date" value={filters.date} onChange={filters.setDate} /></div>
    {rows.length ? <Table rowKey="id" dataSource={rows} pagination={{ pageSize: 10 }} scroll={{ x: 1600 }} columns={[
      { title: "参与用户", width: 200, render: (_: unknown, row: MarketingParticipation) => <Button theme="borderless" size="small" onClick={() => setSelectedId(row.id)}>{participantDisplayName(row, members)}</Button> },
      { title: "参与渠道", width: 140, render: (_: unknown, row: MarketingParticipation) => channelLabels[participantChannel(row)] },
      { title: "手机号", width: 140, render: (_: unknown, row: MarketingParticipation) => maskedPhone(participantIdentity(row, members).phone) },
      { title: "会员状态", width: 160, render: (_: unknown, row: MarketingParticipation) => participationIssue(row, members) || participantIdentityReview(row, members, activity.brand) === "UNVERIFIED" ? "身份待核验" : participantIdentity(row, members).memberId ? "已关联会员" : "未关联会员" },
...
export function BookingData({ activity }: { activity: MarketingActivity }) {
  ...
  const rows = state.bookings.filter((row) => row.activityId === activity.id && (kind === "ALL" || row.kind === kind) && (filters.status === "ALL" || bookingStatus(row, slotFor(state, row), Date.now()) === filters.status) && dateMatches(row.createdAt, filters.date) && (() => { const participant = state.participations.find((item) => item.id === row.participationId); return Boolean(participant && `${participantDisplayName(participant, members)} ${participant.id} ${participant.identities.map((ref) => ref.userId).join(" ")}`.toLowerCase().includes(filters.search.toLowerCase())); })());
...
export function VirtualAwardContent({ award }: { award: MarketingAward }) {
  ...
  if (needsReservation(award) && !award.issuedAt) return <DefinitionGrid rows={[["内容状态", awardFulfillmentLabel(state, award, Date.now())], ["领取方式", "预约履约"], ["奖品内容", "履约完成后显示"], ["有效期", `${displayDate(award.claimStart)} 至 ${displayDate(award.claimEnd)}`]]} />;
  return <>
...
export function AwardData({ activity }: { activity: MarketingActivity }) {
  ...
  return <><Table rowKey="id" dataSource={rows} pagination={{ pageSize: 10 }} scroll={{ x: 1480 }} columns={[
    { title: "用户", width: 170, render: (_: unknown, row: MarketingAward) => (() => { const participant = state.participations.find((item) => item.id === row.participationId); return participant ? participantDisplayName(participant, members) : "身份待核对"; })() },
    ...
    { title: "虚拟内容状态", width: 170, render: (_: unknown, row: MarketingAward) => row.prizeType !== "VIRTUAL" ? "不适用" : needsReservation(row) && !row.issuedAt ? "待履约后展示" : row.virtualContent?.code ? "已分配（详情可见）" : row.issuedAt ? "已保存（详情可见）" : "待核对" },
...
export function RedemptionData({ activity }: { activity: MarketingActivity }) {
  ...
  return <Panel title="核销记录"><SelectField label="核销业务类型" value={type} onChange={setType} list={[{ value: "ALL", label: "全部核销类型" }, ...Object.entries(redemptionLabels).map(([value, label]) => ({ value, label }))]} />
    <Table rowKey="id" dataSource={rows} pagination={{ pageSize: 10 }} scroll={{ x: 1700 }} columns={[
      { title: "用户", width: 170, render: (_: unknown, row: MarketingRedemption) => (() => { const participant = state.participations.find((item) => item.id === row.participationId); return participant ? participantDisplayName(participant, members) : "身份待核对"; })() },
