...
import { displayDate } from "./MarketingUi";
import { codeInventory, participantDisplayName } from "./marketing-model";
...
export function CodeManager({ prize, published, remainingQuota, canManage, onDelete, onClose }: {
  ...
  return <SideSheet visible closeOnEsc title={`兑换码管理 · ${prize.name}`} width={Math.min(780, window.innerWidth - 20)} onCancel={onClose}>
    ...
    {prize.codes.length ? <Table size="small" rowKey="code" dataSource={prize.codes} pagination={{ pageSize: 10 }} scroll={{ x: 720 }} columns={[
      ...
      { title: "分配时间", width: 160, render: (_: unknown, row: MarketingCode) => displayDate(row.assignedAt) },
      { title: "关联用户", width: 130, render: (_: unknown, row: MarketingCode) => {
        ...
        const participant = state.participations.find((participant) => participant.id === award?.participationId);
