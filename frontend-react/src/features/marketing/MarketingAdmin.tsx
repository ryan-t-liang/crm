...
import { activityCreationIssue, activityMetrics, claimLabels, codeInventory, fulfillmentCapacity, marketingPermissions, marketingStatusLabels, needsReservation, prizeTypeLabels, quota, slotOccupancy, winnable } from "./marketing-model";
...
export function MarketingList({ migratedEntry }: { migratedEntry?: string }) {
  ...
  return <>
    <PageHeader title="营销活动" description="以活动为中心管理配置与业务数据；现场签到、完成确认和实体领奖由独立核销端执行。" actions={<Button theme="solid" disabled={Boolean(creationIssue)} onClick={() => { setEditingStep(0); setEditing(createMarketingActivity(access.brands[0], Date.now())); }}>新建活动</Button>} />
  ...
  return <>{feedback}<Panel title="抽奖规则"><DataList rows={[["抽奖期", `${displayDate(activity.lotteryStart)} 至 ${displayDate(activity.lotteryEnd)}`], ["完成发放", `${activity.grantCount}次（首次完成）`], ["累计抽奖上限", activity.drawLimit], ["每日上限", activity.dailyLimit === null ? "不限制" : activity.dailyLimit], ["累计中奖上限", `${activity.winLimit}次；达到后保留剩余机会但停止抽奖`], ["概率合计", `${activity.pool.reduce((sum, item) => sum + item.probability, activity.noWinProbability)}%（含未中奖${activity.noWinProbability}%）`]]} /></Panel>
    <Panel title="当前活动奖品" note="中奖即占用本活动库存，领取不二次扣减。预约型奖品须为尚未预约的赢家保留履约容量；普通列表不展示完整兑换码。">
      <Table rowKey="id" dataSource={activity.pool} pagination={{ pageSize: 10 }} scroll={{ x: 1460 }} columns={[
        ...
        { title: "操作", width: 310, fixed: "right", render: (_: unknown, item: ActivityPrize) => <div className="button-row"><Button size="small" disabled={!access.manage} onClick={() => setEditing(structuredClone(item))}>编辑奖品</Button>{item.method === "REDEMPTION_CODE" && <><Button size="small" disabled={!access.manage} onClick={() => setImportId(item.id)}>导入兑换码</Button><Button size="small" disabled={!access.manage} onClick={() => setCodesId(item.id)}>查看兑换码</Button></>}{activity.publishedAt && <Button size="small" disabled={!access.manage} onClick={() => setExtra({ itemId: item.id, count: 1 })}>增加配额</Button>}{needsReservation(item) && activity.publishedAt && <Button size="small" disabled={!access.manage} onClick={() => setAddingSlot({ itemId: item.id, slot: createMarketingSlot(crypto.randomUUID(), item.claimStart) })}>追加履约时段</Button>}{!activity.publishedAt && <Button size="small" type="danger" disabled={!access.manage} onClick={() => run({ type: "DELETE_ACTIVITY_PRIZE", activityId: activity.id, poolItemId: item.id })}>删除奖品</Button>}</div> },
      ]} />{!activity.publishedAt && <Button size="small" disabled={!access.manage} onClick={() => setEditing(createActivityPrize(activity.id, Date.now()))}>添加奖品</Button>}
    </Panel>
    ...
    {codePrize && access.manage && <CodeManager prize={codePrize} published={Boolean(activity.publishedAt)} remainingQuota={quota(state, activity.id, codePrize).available} canManage={access.manage} onClose={() => setCodesId("")} onDelete={(codes) => run({ type: "DELETE_CODES", activityId: activity.id, poolItemId: codePrize.id, codes })} />}
...
export function MarketingDetail({ activity, requestedTab }: { activity: MarketingActivity; requestedTab?: string }) {
  ...
  const [editingStep, setEditingStep] = useState<number | null>(null), [publishing, setPublishing] = useState(false), [detail, setDetail] = useState<{ title: string; rows: { id: string }[]; at: number } | null>(null);
  ...
  return <><DetailWorkspace eyebrow="会员与品牌运营 / 营销活动 / 活动管理" title={activity.name} subtitle={`${brandLabels[activity.brand]} · ${activity.mode === "OFFLINE" ? "线下活动" : "线上活动"} · 免费活动`} backRoute="marketing" tags={<Tag>{marketingStatusLabels[activity.status]}</Tag>} actions={<>
    <Button disabled={!access.manage} onClick={() => setEditingStep(0)}>编辑活动</Button><Button disabled={!access.preview} onClick={() => navigate(`marketing/preview/${activity.id}`)}>用户流程预览</Button><Button disabled={!access.manage} onClick={() => { const result = run({ type: "COPY_ACTIVITY", activityId: activity.id }); if (result.ok) navigate(`marketing/activity/${result.resultId}`); }}>复制活动</Button>
    {activity.status !== "CANCELED" && <Button disabled={!access.manage} onClick={() => { if (activity.publishedAt) run({ type: "STATUS", activityId: activity.id, status: activity.status === "PUBLISHED" ? "PAUSED" : "PUBLISHED" }); else setPublishing(true); }}>{activity.status === "PUBLISHED" ? "暂停" : activity.status === "PAUSED" ? "恢复" : "发布"}</Button>}
    ...
    <Tabs activeKey={tab} onChange={(key) => navigate(`marketing/activity/${activity.id}/${key}`)}>
      ...
      {activity.bookingEnabled && <TabPane itemKey="booking-settings" tab="预约设置"><Button size="small" disabled={!access.manage || Boolean(activity.publishedAt)} onClick={() => setEditingStep(1)}>编辑预约规则</Button><BookingSettings activity={activity} /></TabPane>}
      {activity.lotteryEnabled && <TabPane itemKey="lottery" tab="抽奖 & 奖品"><Button size="small" disabled={!access.manage || Boolean(activity.publishedAt)} onClick={() => setEditingStep(2)}>编辑抽奖规则</Button><PrizeSettings activity={activity} /></TabPane>}
