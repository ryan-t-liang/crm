  ...
  try {
    ...
    const shapes = objects && parsed.activities.every((row) =>
      ...
      parsed.participations.every((row) => typeof row.activityId === "string" && typeof row.subjectKey === "string" && typeof row.credential === "string" && Array.isArray(row.identities) && row.identities.every((ref) => typeof ref.userId === "string" && typeof ref.brand === "string") &&
        ...
        (row.taskClues === undefined || Array.isArray(row.taskClues) && row.taskClues.length === 4 && new Set(row.taskClues.map((clue) => clue.id)).size === 4 && row.taskClues.every((clue) => typeof clue.id === "string" && typeof clue.label === "string" && typeof clue.completed === "boolean" && (clue.completedAt === undefined || typeof clue.completedAt === "string"))) &&
