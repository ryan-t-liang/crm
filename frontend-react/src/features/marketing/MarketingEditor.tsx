...
import { claimLabels, codeInventory, marketingPermissions, needsReservation, prizeErrors, prizeTypeLabels, validateActivity } from "./marketing-model";
...
export function CodeImporter({ onImport }: { onImport: (codes: string[]) => boolean }) {
  const [text, setText] = useState(""), [error, setError] = useState("");
  const submit = () => { try { if (onImport(parseMarketingCodes(text))) { setText(""); setError(""); } } catch (error) { setError(error instanceof Error ? error.message : "读取失败"); } };
  return <Panel title="批量导入兑换码" note="每行一个兑换码；CSV仅支持一列，可带code或兑换码表头。重复或非法值整批拒绝，不部分导入。">
    ...
    {error && <Banner type="warning" title={error} closeIcon={null} />}<Button size="small" onClick={submit}>确认导入兑换码</Button>
  </Panel>;
}
...
export function PrizeFields({ prize, onChange }: { prize: ActivityPrize; onChange: (prize: ActivityPrize) => void }) {
  ...
  const changeType = (type: string) => onChange({ ...prize, prizeType: type as ActivityPrize["prizeType"], method: type === "VIRTUAL" ? "REDEMPTION_CODE" : "DIRECT", location: type === "VIRTUAL" ? "" : "演示工作室", slots: [], codes: [], voucherName: "", voucherDescription: "", link: "" });
  const changeMethod = (method: string) => onChange({ ...prize, method: method as ActivityPrize["method"], slots: ["PICKUP", "EXPERIENCE"].includes(method) ? prize.slots.length ? prize.slots : [createMarketingSlot(crypto.randomUUID(), Date.now(), Math.max(1, prize.quota))] : [] });
  return <>
    ...
    {needsReservation(prize) && <Panel title="奖品履约时段" note="与活动参加场次独立；中奖前会预留未预约赢家的履约承诺，不得超出可履约容量。">
      ...
      <Button size="small" onClick={() => update("slots", [...prize.slots, createMarketingSlot(crypto.randomUUID(), Date.now())])}>添加履约时段</Button>
    </Panel>}
    ...
    {prize.prizeType === "VIRTUAL" && prize.method === "REDEMPTION_CODE" && <>
      ...
      {error && <Banner type="warning" title={error} closeIcon={null} />}
      <CodeImporter onImport={(codes) => { if (codes.some((code) => prize.codes.some((row) => row.code === code))) { setError("兑换码已存在；整批未导入"); return false; } update("codes", [...prize.codes, ...codes.map((code) => ({ code }))]); setError(""); return true; }} />
    </>}
...
export function ActivityEditor({ initial, onClose, initialStep = 0 }: { initial: MarketingActivity; onClose: () => void; initialStep?: number }) {
  ...
  return <SideSheet visible closeOnEsc width={Math.min(780, window.innerWidth - 20)} title={locked ? "编辑活动说明（规则已锁定）" : "新建 / 编辑活动"} onCancel={onClose} footer={<div className="button-row"><Button onClick={onClose}>取消</Button>{!locked && step > 0 && <Button onClick={() => setStep(step - 1)}>上一步</Button>}{!locked && step < 4 && <Button onClick={() => setStep(step + 1)}>下一步</Button>}<Button onClick={() => save(false)}>{locked ? "保存说明" : "保存草稿"}</Button>{!locked && <Button theme="solid" onClick={() => save(true)}>保存并发布</Button>}</div>}>
    <div className="marketing-editor"><DemoNote />{feedback}{error && <Banner type="warning" title={error} closeIcon={null} />}
      ...
      {!locked && step === 4 && <Panel title="发布检查" note="保存草稿允许缺少文字或时间配置，但非法库存和概率值不能保存。发布前需完整通过。">{errors.length ? <ul>{errors.map((item) => <li key={item}>{item}</li>)}</ul> : <p>配置检查通过，等待发布。</p>}<p>奖品概率 + 未中奖概率合计100%；不自动重新归一化。预约型奖品还受履约容量限制，兑换码须足额导入。</p></Panel>}
    </div>
  </SideSheet>;
}
