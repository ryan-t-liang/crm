...
import { bookingStatus, canViewMarketing, chances, claimLabels, drawBlock, isAwardFulfilled, marketingPermissions, needsReservation, participantChannel, participantDisplayName, participantIdentity, readActivityRule, participationFor, participationIssue, resolveCredential, slotFor, slotOccupancy } from "./marketing-model";
...
import { ActivityPhases, DefinitionGrid, displayDate, displayAwardStatus as awardFulfillmentLabel, displayBookingLabels as bookingLabels, displayChannelLabels as participationChannelLabels, prizeReceivingLabel, Panel, SelectField, TextField, options, useAction } from "./MarketingUi";
...
function BookingTable({ rows }: { rows: MarketingBooking[] }) {
  const { state } = useMarketing(), { state: members } = useMemberOperations(), { run, feedback } = useAction(); const [change, setChange] = useState<MarketingBooking | null>(null), [target, setTarget] = useState("");
...
function MarketingPreview({ activity, initialUserId }: { activity: MarketingActivity; initialUserId?: string }) {
  ...
  return <div className="page marketing-preview"><Button theme="borderless" onClick={() => navigate("marketing/activity/" + activity.id)}>返回活动后台</Button><PageHeader title="用户流程预览" description={activity.name + " · " + brandLabels[activity.brand]} />
    ...
    {activity.lotteryEnabled && <Panel title="完成后抽奖"><DefinitionGrid rows={[["累计获得", balance.earned], ["已使用", balance.used], ["剩余", balance.remaining], ["活动抽奖上限", activity.drawLimit], ["当前状态", reason || "可抽奖"]]} />
      ...
      {last && <p className="marketing-draw-result" role="status">最近已保存结果：{last.poolItemId ? state.awards.find((award) => award.drawId === last.id)?.prizeName : "未中奖"}</p>}
    </Panel>}
