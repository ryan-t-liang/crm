...
export function PickupRoster({ activity, rows }: { activity: MarketingActivity; rows: MarketingBooking[] }) {
  ...
  return <Table rowKey="id" dataSource={rows} pagination={{ pageSize: 10 }} scroll={{ x: 710 }} empty={<EmptyBlock title="暂无预约记录" description="用户提交领奖预约后在此展示。" />} columns={[
    ...
    { title: "领取时段", width: 155, render: (_: unknown, row: MarketingBooking) => { const slot = pickupSlotForBooking(activity, row); return slot ? <DateRange start={slot.startAt} end={slot.endAt} /> : "时段待核对"; } },
...
function ScheduleEditor({ activity, initial, onClose }: { activity: MarketingActivity; initial?: MarketingPickupSchedule; onClose: () => void }) {
  ...
  return <FormSideSheet visible title={initial ? "编辑领取安排" : "新建领取安排"} width={640} onCancel={onClose} onOk={() => { if (run({ type: "SAVE_PICKUP_SCHEDULE", activityId: activity.id, schedule: form }).ok) onClose(); }}>
    {feedback}<div className="form-grid">
      <TextField label="领取安排名称" value={form.name} onChange={name => setForm({ ...form, name })} />
    ...
    </div><p className="marketing-field-help">保存后可批量生成领取时段。多个奖品绑定此安排时共享预约容量；已有预约的地点和时段不会被覆盖。</p>
  </FormSideSheet>;
}
...
function BatchSlots({ activity, schedule, onClose }: { activity: MarketingActivity; schedule: MarketingPickupSchedule; onClose: () => void }) {
  ...
  return <Modal visible maskClosable={false} width={720} title="批量生成领取时段" onCancel={onClose} bodyStyle={{ maxHeight: "calc(100vh - 210px)", overflowY: "auto" }} footer={<div className="sheet-footer"><Button onClick={onClose}>{result ? "完成" : "取消"}</Button>{!result && <Button theme="solid" onClick={() => {
    ...
    {feedback}{result ? <Banner type="success" title={result} closeIcon={null} /> : <><div className="form-grid">
      ...
      <NumberField label="每个时段预约容量" value={input.capacity} onChange={capacity => update({ capacity })} />
      ...
      {preview.error ? <Banner type="warning" title={preview.error} closeIcon={null} /> : <Table rowKey="id" dataSource={preview.slots} pagination={{ pageSize: 5 }} columns={[
        ...
        { title: "地点", dataIndex: "location" }, { title: "预约容量", dataIndex: "capacity" },
      ]} />}
    </Panel>}</>}
  </Modal>;
}
...
function SlotEditor({ activity, schedule, slot, onClose }: { activity: MarketingActivity; schedule: MarketingPickupSchedule; slot: MarketingSlot; onClose: () => void }) {
  ...
  return <FormSideSheet visible title="编辑领取时段" width={640} onCancel={onClose} onOk={() => { if (run({ type: "SAVE_PICKUP_SLOT", activityId: activity.id, scheduleId: schedule.id, slot: form }).ok) onClose(); }}>
    ...
    <div className="form-grid">
      ...
      <NumberField label="预约容量" disabled={history} value={form.capacity} onChange={capacity => setForm({ ...form, capacity })} />
...
export function PickupScheduleDrawer({ activity, scheduleId, onClose }: { activity: MarketingActivity; scheduleId: string; onClose: () => void }) {
  ...
  if (!schedule) return <SideSheet visible title="领取安排待核对" onCancel={onClose}><EmptyBlock title="领取安排不存在" /></SideSheet>;
  ...
  return <>
    <SideSheet visible={!edit && !batch && !slot && !adjust} closeOnEsc title={schedule.name} width={Math.min(820, window.innerWidth - 24)} onCancel={onClose} footer={<Button onClick={onClose}>关闭</Button>}>
      {feedback}<Panel title="领取安排" actions={<div className="row-actions"><Button size="small" disabled={!manage} onClick={() => setEdit(true)}>编辑</Button><Button size="small" disabled={!manage} onClick={() => setBatch(true)}>批量生成时段</Button></div>}>
        <DataList rows={[["地点", schedule.location], ["有效日期", displayDateRange(schedule.startAt, schedule.endAt).compact], ["关联奖品", summary.prizes.map(prize => prize.name).join("、") || "未关联"], ["预约容量 / 已预约 / 剩余", `${summary.total} / ${summary.booked} / ${summary.remaining}`]]} />
      </Panel>
      {summary.warning && <Banner type="warning" title={summary.warning} closeIcon={null} />}
      <Panel title="领取时段"><Table rowKey="id" dataSource={schedule.slots.filter(row => !row.deleted)} pagination={{ pageSize: 8 }} scroll={{ x: 730 }} empty={<EmptyBlock title="暂无领取时段" description="通过批量生成时段添加可预约时间。" />} columns={[
        ...
        { title: "预约容量", width: 160, render: (_: unknown, row: MarketingSlot) => { const used = pickupBookedCount(state, activity, schedule.id, row.id); return <div className="marketing-summary-cell"><span>总容量 {row.capacity}</span><small>已预约 {used} · 剩余 {Math.max(0, row.capacity - used)}</small></div>; } },
        ...
        ]}><Button theme="borderless" size="small" icon={<IconMore />} aria-label="更多领取时段操作" /></MarketingMenu></div> },
      ]} /></Panel>
    ...
    {adjust && adjustingSlot && <Modal visible title="调整预约容量" maskClosable={false} width={520} onCancel={() => setAdjust(null)} onOk={() => { if (run({ type: "ADJUST_PICKUP_CAPACITY", activityId: activity.id, scheduleId: schedule.id, slotId: adjust.id, capacity: adjust.capacity }).ok) setAdjust(null); }}>
      {feedback}<DataList rows={[["领取安排", schedule.name], ["时段", displayDateRange(adjustingSlot.startAt, adjustingSlot.endAt).compact], ["当前容量", String(adjustingSlot.capacity)], ["已预约", String(booked)], ["当前剩余", String(Math.max(0, adjustingSlot.capacity - booked))]]} />
...
export function PickupScheduleSection({ activity, openId, onOpen, onClose }: { activity: MarketingActivity; openId: string; onOpen: (id: string) => void; onClose: () => void }) {
  ...
  return <><Panel title="领取安排" actions={<Button size="small" icon={<IconPlus />} disabled={!marketingPermissions(currentUser).manage} onClick={() => setCreating(true)}>新建领取安排</Button>}>
    <Table rowKey="id" dataSource={pickupSchedules(activity)} pagination={{ pageSize: 5 }} scroll={{ x: 760 }} empty={<EmptyBlock title="暂无领取安排" description="为预约领取的奖品创建可共享的领取时段与容量。" />} columns={[
      { title: "领取安排 / 地点", width: 210, render: (_: unknown, row: MarketingPickupSchedule) => <div className="marketing-summary-cell"><Button theme="borderless" size="small" onClick={() => onOpen(row.id)}>{row.name}</Button><small>{row.location}</small></div> },
      { title: "有效日期", width: 180, render: (_: unknown, row: MarketingPickupSchedule) => <DateRange start={row.startAt} end={row.endAt} /> },
      { title: "预约容量", width: 170, render: (_: unknown, row: MarketingPickupSchedule) => { const value = pickupScheduleSummary(state, activity, row, Date.now()); return <div className="marketing-summary-cell"><span>总容量 {value.total}</span><small>已预约 {value.booked} · 剩余 {value.remaining}</small></div>; } },
      { title: "关联奖品 / 状态", width: 140, render: (_: unknown, row: MarketingPickupSchedule) => { const value = pickupScheduleSummary(state, activity, row, Date.now()); return <div className="marketing-summary-cell"><span>{value.prizes.length} 个奖品</span><Tag size="small">{value.status}</Tag></div>; } },
